...
export type PageTree = Page | Section | CollapsibleGroup | SidebarAction;

// === SidebarAction: A button that opens a Dialog or Sheet (not a page) ===

export type SidebarAction = {
  readonly _tag: "SidebarAction";
  name: string;
  slug: Slug;
  icon?: LucideIcon;
  /** The dialog/sheet content to render when triggered */
  actionContent: ReactNode;
  /** Dialog or Sheet variant */
  actionType: "dialog" | "sheet";
...
export type SidebarActionConfig = {
  name: string;
  slug: Slug;
  icon?: LucideIcon;
  actionContent: ReactNode;
...
const sidebarAction = (config: SidebarActionConfig): SidebarAction => ({
  _tag: "SidebarAction",
  name: config.name,
  slug: config.slug,
  icon: config.icon,
...
- **Add `SidebarAction` type and `sidebarAction()` factory** for dialog/sheet triggers in sidebar
...
### SidebarAction Navigation Behavior

`SidebarAction` nodes do not navigate - they trigger a dialog or sheet. In `pageTree.find()`:
- Returns `none` for `SidebarAction` nodes (they are not pages)
- They are excluded from breadcrumbs
- In `pageTree.findBySlug()`, returns `none` for `SidebarAction` nodes

The `actionContent` and `actionType` fields are used by the rendering layer to open the appropriate overlay component when the sidebar button is clicked.
...
export { page, subPage, collapsibleGroup, section, sidebarAction } from "./page.js";
export type { Page, SubPage, Section, CollapsibleGroup, SidebarAction, PageTree, BreadcrumbSegment, TraversalResult, Maybe, Slug, PageConfig, SubPageConfig, CollapsibleGroupConfig, SectionConfig, SidebarActionConfig } from "./page.js";
```
...
```typescript
import { defineConfig, page, section, sidebarAction, pageTree } from "deesse";
...
### 9.1 SidebarAction Example (Dialog/Sheet Trigger)

```typescript
import { defineConfig, section, sidebarAction } from "deesse";
...
const config = defineConfig({
  ...
  pages: [
    section({
      ...
      slug: "users",
      children: [
        page({ name: "List", slug: "list", content: <UsersPage /> }),
        // A sidebar button that opens a dialog to create a new user
        sidebarAction({
          name: "Add User",
          slug: "add-user",
          icon: Plus,
        ...
        // A sidebar button that opens a sheet for quick actions
        sidebarAction({
          name: "Export",
          slug: "export",
          icon: Download,
...
**Note:** `SidebarAction` nodes do not navigate - clicking them opens the `actionContent` in a Dialog or Sheet overlay. The slug is used for identification but does not correspond to a route.
...
| File | Action |
|------|--------|
| `packages/deesse/src/config/page.ts` | Update types with `_tag` discriminant, add `position`, make `content` non-nullable, add `SidebarAction` type |
...
| `packages/deesse/src/config/index.ts` | Export new types including `SidebarAction` and `sidebarAction()` |
